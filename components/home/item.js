...
export default class Item extends Component {
 	...
 	showTube(item){
 		appNav({action:'push',nav:'appNav',name:'chat',info:{title:this.props.item.contacts[0].fullName,item:item}})
  ...
  getImageURI(chat) {
    if (chat.isGroupChat) {
      ...
      profilePics = chat.contacts[0].profilePics.filtered('primary = true')
			...
			<View style={{flexDirection:'row',padding:10,alignItems:'center',paddingTop:2,paddingBottom:0}}>
				...
				<View style={{marginLeft:4,width:k>1?255*k:245*k,padding:7,paddingRight:10,height:72,}}>
					<View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
						...
						<Text style={{color:TEXT_GREY}}>{this.showLastMessageTime(this.props.item)}</Text>
					</View>
					<View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
						<Text style={{width:200*k,color:TEXT_GREY, fontWeight: this.props.item.notificationCount > 0 ? 'bold' : 'normal'}}>
